  ...
  pub fn active_player_index(&self) -> Result<PlayerIndex> {
    match &self.active_turn {
      ...
      Turn::Call(call) => {
        ...
        if self.has_gathered_resources() && c.markers.iter().any(|m| *m == Marker::Bewitched) {
          self
            .characters
            .get(Rank::One)
            .player
    ...
    match self.active_turn.borrow() {
      ...
      Turn::Draft(Draft {
        ...
        if self
          .turn_actions
          .iter()
          .any(|act| act.tag() == ActionTag::Theater || act.tag() == ActionTag::TheaterPass)
        {
          vec![]
        } else {
  ...
  fn perform_action(&mut self, action: &Action) -> ActionResult {
    Ok(match action {
      ...
      Action::RevealBlackmail => match self.followup {
        Some(Followup::Blackmail { blackmailer }) => {
          ...
          if is_flowered {
            ...
            for c in self.characters.iter_mut() {
              if let Some((i, _)) = c.markers.iter().enumerate().find(|(_, m)| m.is_blackmail()) {
                c.markers.remove(i);
              }
            }
            ...
            ActionOutput::new(format!(
              ...
              self.players[blackmailer.0].name, gold
            ))
          } else {
      ...
      Action::DraftPick { role } => {
        ...
        if self.players.len() == 2 && (draft.remaining.len() == 5 || draft.remaining.len() == 3) {
          output
        } else {
          output.end_turn()
        }
      ...
      Action::EndTurn => {
        //
        ActionOutput::new(format!("{} ends their turn.", self.active_player()?.name)).end_turn()
      }
      ...
      Action::GatherCardsPick { district } => {
        let mut revealed =
          if let Some(Followup::GatherCardsPick { revealed, .. }) = self.followup.borrow_mut() {
            revealed
          } else {
  ...
  fn end_turn(&mut self) -> Result<()> {
    ...
    match self.active_turn.borrow_mut() {
      ...
      Turn::Draft(draft) => {
        ...
        // randomly discard 1.
        if self.players.len() == 3 && self.characters.len() == 9 && draft.remaining.len() == 5 {
      ...
      Turn::Call(_) => {
        if let Ok(player) = self.active_player() {
          ...
          let poor_house =
            role != RoleName::Witch && player.gold == 0 && player.city_has(DistrictName::PoorHouse);
          ...
          if refund > 0 {
            ...
            self
              ...
              .unwrap()
              .logs
              .push(format!("The Alchemist is refunded {} gold spent building.", refund).into());
          }
        }
        self.call_next();
      }
