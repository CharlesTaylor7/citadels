use crate::game::{CityDistrict, Game, Followup, ActionOutput, ActionResult, Result, PlayerIndex};
...
pub fn perform_action(game: &mut Game, action: &Action) -> ActionResult {
        Ok(match action {
            ...
            Action::EndTurn => {
                //
                ActionOutput::new(format!("{} ends their turn.", game.active_player()?.name))
                    .end_turn()
            }
            ...
            Action::WizardPick(method) => match game.followup {
                Some(Followup::WizardPick { player: target }) => {
                        ...
                        && !game.turn_actions.iter().any(|act| act.is_build())
                    {
                        ...
                        game.complete_build(
                            game.active_player().unwrap().index,
                            cost,
                            district.name,
                        );
